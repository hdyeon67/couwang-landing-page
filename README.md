...
문의 링크는 아직 확정 전이라 `js/main.js` 상단의 `CONTACT_URL`에서 관리합니다.

```js
const CONTACT_URL = "#";
```

크몽, 숨고, 위시켓, 이메일 주소가 확정되면 아래처럼 교체합니다.

```js
const CONTACT_URL = "mailto:hello@example.com";
```

또는:

```js
const CONTACT_URL = "https://kmong.com/...";
```

외부 문의 링크를 사용할 경우 필요에 따라 `index.html`의 문의 링크에 `target="_blank"`와 `rel="noopener noreferrer"`를 추가합니다.
...
`og-image.png`가 없다면 1200 x 630 비율로 생성해 `assets/` 폴더에 추가합니다. 쿠왕 로고, 마스코트, 앱 대표 화면이 함께 보이는 구성이 적합합니다.

favicon은 현재 `assets/logo.png`를 사용합니다.

```html
<link rel="icon" href="assets/logo.png">
```

별도 favicon을 사용하려면 `assets/favicon.ico` 또는 `assets/favicon.png`를 추가하고 위 경로를 수정합니다.
