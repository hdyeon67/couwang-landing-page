...
- 앱 스토어 링크 변경: `index.html`에서 Google Play 또는 App Store URL을 검색해 Hero와 Footer 링크를 함께 수정합니다.
- 앱 운영 문의 이메일 변경: `index.html`에서 `mailto:fineboll67@gmail.com`을 검색해 교체합니다.
- 인스타그램 링크 변경: `index.html`에서 `https://www.instagram.com/couwang.app/`를 검색해 교체합니다.
- 공유 이미지 변경: `assets/og-image.png`를 같은 파일명으로 교체합니다.
- favicon 변경: `assets/favicon.png`를 같은 파일명으로 교체합니다.
- 앱 화면 이미지 변경: `assets/app-*.png` 파일을 같은 파일명으로 교체합니다.
- 카카오톡/SNS 공유 미리보기 확인: 배포 후 실제 URL을 카카오톡, X, Slack 등에 붙여 넣어 제목, 설명, 이미지가 의도대로 보이는지 확인합니다.
...
앱 스크린샷은 현재 `828 x 1792` 세로형 이미지를 기준으로 `#screens` 섹션에서 잘리지 않도록 표시합니다. 다른 비율의 이미지를 넣을 경우 [css/style.css](css/style.css)의 `.screen-card img`에 있는 `aspect-ratio`를 함께 조정합니다.

스크린샷 이미지 용량이 커질 경우 WebP 변환을 권장합니다. 단, 파일 확장자나 파일명을 바꾸면 `index.html`의 이미지 경로도 함께 바꿔야 합니다.

## 디자인 표시 기준

- 브랜드 컬러는 `#64CAFA`를 기준으로 유지합니다.
- 헤더와 푸터의 `logo.png`는 CSS에서 적당한 라운드와 shadow를 적용해 표시합니다.
- `favicon.png`는 파일 자체에 라운드 처리가 반영되어 있으며, favicon과 Apple Touch Icon에 함께 사용합니다.
- 앱 화면 이미지는 카드 안에서 `object-fit: contain`으로 표시해 화면 내용이 잘리지 않게 유지합니다.
- 외주 제작 문의는 별도 CTA 버튼으로 과하게 노출하지 않고, 플랫폼 메시지 안내 문구로만 표시합니다.
...
이메일이 아직 확정되지 않은 새 프로젝트에서는 아래 형식으로 placeholder를 둘 수 있습니다.

```html
mailto:YOUR_EMAIL@example.com?subject=쿠왕 앱 문의
```
...
카카오톡/SNS 공유 미리보기는 배포 URL을 직접 공유해 확인합니다. 이미지나 문구를 바꿨는데 미리보기가 바로 갱신되지 않으면 각 플랫폼의 캐시 때문에 이전 정보가 보일 수 있으므로 잠시 후 다시 확인하거나, 플랫폼에서 제공하는 공유 디버거를 사용합니다.
...
- 상품 2 포트폴리오 캡처에는 Hero, App Screens, Project Story, Contact 섹션이 함께 보이도록 사용
...
- [ ] OG 메타태그 입력 완료
- [ ] Twitter 카드 메타태그 입력 완료
- [ ] 카카오톡/SNS 공유 미리보기 확인 완료
